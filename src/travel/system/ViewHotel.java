...
public class ViewHotel extends JFrame implements ActionListener{
    ...
    JLabel userfield,dayfield,phfield,emfield,htlfield,t,ppfield,acfield;
    String username;
    JButton back;
    ViewHotel(String username){
        ...
        JLabel htl=new JLabel("Hotel Name");
        htl.setBounds(50,120,150,25);
        add(htl);
        
        htlfield=new JLabel();
        htlfield.setBounds(200,120,150,25);
        add(htlfield);
        
        JLabel no=new JLabel("Total heads");
        no.setBounds(50,160,150,25);
        add(no);
        
        t=new JLabel();
        t.setBounds(200,160,150,25);
        add(t);
        
        JLabel udays=new JLabel("No. of days");
        udays.setBounds(50,200,150,25);
        add(udays);

        dayfield=new JLabel();
        dayfield.setBounds(200,200,150,25);
        add(dayfield);
        
        JLabel uac=new JLabel("AC Room");
        uac.setBounds(50,240,150,25);
        add(uac);

        acfield=new JLabel();
        acfield.setBounds(200,240,150,25);
        add(acfield);
        
        JLabel uph=new JLabel("Phone No");
        uph.setBounds(50,280,150,25);
        add(uph);

        phfield=new JLabel();
        phfield.setBounds(200,280,150,25);
        add(phfield);
        
        JLabel uem=new JLabel("Email");
        uem.setBounds(50,320,150,25);
        add(uem);

        emfield=new JLabel();
        emfield.setBounds(200,320,150,25);
        add(emfield);
        
        JLabel uprice=new JLabel("Total Price");
        uprice.setBounds(50,360,150,25);
        add(uprice);
        
        ppfield=new JLabel();
        ppfield.setBounds(200,360,150,25);
        add(ppfield);
        ...
        try{
            ...
            while(rs.next()){
                ...
                htlfield.setText(rs.getString("hotelname"));
                phfield.setText(rs.getString("phone"));
                emfield.setText(rs.getString("email"));
                dayfield.setText(rs.getString("Nod"));
